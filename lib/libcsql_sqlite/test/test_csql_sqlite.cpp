...
BOOST_AUTO_TEST_SUITE(test_csql)

BOOST_AUTO_TEST_CASE(test_csql_sqlite_create_and_drop_table)
{
...
BOOST_AUTO_TEST_CASE(test_csql_sqlite_insert_rows)
{
    SqliteDatabase db{"my.db"};
...
BOOST_AUTO_TEST_CASE(test_csql_sqlite_insert_and_count_rows)
{
    SqliteDatabase db{"my.db"};
    MyTableInfo tbl{db};
    ...
    tbl.clearRows();
    tbl.selectRows(nullptr, {}, {}, {}, {}, 0, 0, [](const size_t rowsCount) { BOOST_CHECK(rowsCount == 6); });

    BOOST_CHECK(tbl.dropTable());
}

BOOST_AUTO_TEST_CASE(test_csql_sqlite_insert_and_select_all_rows)
{
    SqliteDatabase db{"my.db"};
    ...
    tbl.selectRows(
        [&tbl](const size_t rowIndex) {
            BOOST_CHECK(rowIndex < tbl.ID.getRowsCount() && rowIndex < tbl.C1.getRowsCount() &&
                        rowIndex < tbl.C2.getRowsCount() && rowIndex < tbl.C3.getRowsCount() &&
                        rowIndex < tbl.C4.getRowsCount() && rowIndex < tbl.C5.getRowsCount());
        ...
        {}, {}, {}, {}, 0, 0, [](const size_t rowsCount) { BOOST_CHECK(rowsCount == 5); });

    BOOST_CHECK(tbl.dropTable());
}

BOOST_AUTO_TEST_CASE(test_csql_sqlite_insert_and_select_rows)
{
    SqliteDatabase db{"my.db"};
    ...
    tbl.selectRows(
        ...
        {}, {}, {tbl.ID > 1 && tbl.ID < 5}, {}, 0, 0, [](const size_t rowsCount) { BOOST_CHECK(rowsCount == 3); });

    BOOST_CHECK(tbl.dropTable());
}

BOOST_AUTO_TEST_CASE(test_csql_sqlite_update_and_select_rows)
{
    SqliteDatabase db{"my.db"};
...
BOOST_AUTO_TEST_CASE(test_csql_sqlite_delete_and_select_rows)
{
    SqliteDatabase db{"my.db"};
    ...
    tbl.selectRows(
        ...
        {}, {}, {}, {}, 0, 0, [](const size_t rowsCount) { BOOST_CHECK(rowsCount == 2); });
