...
const SqlName &Table::getTableName() const
{
    ...
    for (size_t rowIndex = 0; rowIndex < this->getMaxRowsCount(); ++rowIndex)
    {
        ...
        for (auto &c : tableColumns)
            if (!c.get().is_AUTOINCREMENT())
            {
                ...
                if (insertValues)
                    sqlBufValues << c.get().getSqlRowValue(rowIndex);
                else
                    sqlBufValues << ":" << c.get().getSqlName('_') << '_' << rowIndex;
                sqlBufValues << ',';
            }
...
SqlExpr Table::getSqlRowsUpdate(const TableColumnComparsionExpr &condition, const size_t count,
...
SqlExpr Table::getSqlRowsSelect(const std::initializer_list<TableColumnBase> &sc,
                                const std::initializer_list<TableJoin> &tableJoins,
                                const TableColumnComparsionExpr &condition,
                                const std::initializer_list<TableRowSort> &sort, const size_t count,
                                const size_t offset) const
{
    std::ostringstream sqlBuf;
    ...
    if (sc.size() > 0)
    {
        ...
        sqlBuf << std::endl << this->tableName << ".*";
    ...
    if (sort.size() > 0)
    {
        ...
        for (auto &s : sort)
        {
            ...
            case TableRowSort::TableRowSortType::DESC:
                sqlBuf << " DESC";
                break;
            case TableRowSort::TableRowSortType::DEFAULT:
            default:
                break;
            }
            sqlBuf << ',';
...
SqlExpr Table::getSqlRowsDelete(const TableColumnComparsionExpr &condition, const size_t count,
                                const size_t offset) const
{
    std::ostringstream sqlBuf;
