...
#include "csql_exception.hpp"
#include "csql_sqlexpr.hpp"
#include "csql_sqlname.hpp"
#include "csql_tablecolumnspecs.hpp"
#include "csql_tablecolumntype.hpp"
#include "csql_tablecomparsionexpr.hpp"
...
public:
    ...
    TableColumnBase(const TCT type, const TCS specs = TCS::EMPTY, const SqlName &name = {})
        : columnType{type}, columnSpecs{specs}, columnName{name}
    {
    }

    virtual TableColumnType getType() const
    {
        return columnType;
    }

    virtual SqlExpr getSqlName(const char delimiter = '.') const;
    virtual SqlExpr getSqlDefaultValue() const
    {
        return {};
    }
    virtual SqlExpr getSqlRowValue(size_t row_index) const
    {
        std::ignore = row_index;
        return {};
    }
    virtual SqlExpr getSqlCreateTable() const
    {
        return {};
    }
    ...
    virtual void addRowStr(const char *rowStr)
    {
        std::ignore = rowStr;
    }
    virtual void addRowPtr(const std::shared_ptr<void> rowPtr)
    {
        std::ignore = rowPtr;
    }
    ...
    {
        return csql::core::is_PRIMARY_KEY(this->columnSpecs);
    }
    virtual bool is_AUTOINCREMENT() const
    {
        return csql::core::is_AUTOINCREMENT(this->columnSpecs);
    }
    virtual bool is_NOT_NULL() const
    {
        return csql::core::is_NOT_NULL(this->columnSpecs);
    }
    virtual bool is_DEFAULT() const
    {
        return csql::core::is_DEFAULT(this->columnSpecs);
    }
    virtual bool is_VALUE_REQUIRED() const
    {
        return is_NOT_NULL() || is_DEFAULT();
    }

    operator TableColumnComparsionExpr() const
    {
        return this->columnName.value == "" ? TableColumnComparsionExpr(this->getSqlDefaultValue())
...
public:
    decltype(tableColumns)::iterator begin()
    {
        return tableColumns.begin();
    }
    decltype(tableColumns)::iterator end()
    {
        return tableColumns.end();
    }
    decltype(tableColumns)::const_iterator begin() const
    {
        return tableColumns.cbegin();
    }
    decltype(tableColumns)::const_iterator end() const
    {
        return tableColumns.cend();
    }
    ...
    template <typename... T> void registerColumns(T &&...columns)
    {
        auto this_sp = std::shared_ptr<Table>(this, [](Table *) {});
        size_t col_index{0};

        tableColumns = {(([&this_sp, &col_index](T &tc) -> T & {
            tc.table = this_sp;
    ...
    SqlExpr getSqlRowsUpdate(const TableColumnComparsionExpr &condition = {}, const size_t count = 0,
                             const size_t offset = 0) const;

    SqlExpr getSqlRowsSelect(const std::initializer_list<TableColumnBase> &sc = {},
    ...
    SqlExpr getSqlRowsDelete(const TableColumnComparsionExpr &condition = {}, const size_t count = 0,
                             const size_t offset = 0) const;
    ...
    template <typename... T> void addRow(T &&...t)
    {
    ...
    template <typename T> void addRow(size_t colIndex, const T val)
    {
        ...
        {
            std::for_each_n(this->tableColumns.begin(), colIndex,
                            [](auto &column) { column.get().removeLastRowPtr(); });
...
struct TableJoin
{
    ...
    TableJoin(const TableJoinType joinType_, const SqlName &tableName_,
              const TableColumnComparsionExpr &joinCondition_ = {})
        : joinType{joinType_}, tableName{tableName_}, joinCondition{joinCondition_}
    {
    }

    TableJoin(const TableJoinType joinType_, const Table &table_, const TableColumnComparsionExpr &joinCondition_ = {})
