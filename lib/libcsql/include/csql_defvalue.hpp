...
public:
    ...
    template <typename T1 = T,
              std::enable_if_t<std::is_arithmetic<T1>::value && !std::is_same<T1, time_t>::value, bool> = true>
    ...
    template <typename T1 = T, std::enable_if_t<std::is_same<T1, std::time_t>::value, bool> = true>
    std::string getSqlValue() const
    {
        std::stringstream out;
